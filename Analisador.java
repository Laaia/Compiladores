...
class Analisador{
  ...
  public void analisa(String linha, int nLinha, String nArquivo){
    ...
    if(linha.matches("^(int(\\s)+|float(\\s)+|double(\\s)+|char(\\s)+)([a-zA-Z])+([0-9])*(\\s)*;$")){
      ...
      addSimbolo(indice, token[1], "Variável", token[0], "Local");
  ...
  public void addSimbolo(int id, String nome, String categoria, String tipo, String escop){
    tabelaSimbolos.add(new Simbolos());
    tabelaSimbolos.get(tabelaSimbolos.size() - 1).setId(id);
    ...
    tabelaSimbolos.get(tabelaSimbolos.size() - 1).setEscopo(escop);
  }
  ...
  public void addToken(int id, String tipo, int posicao){
    ...
    tabelaTokens.get(tabelaTokens.size() - 1).setPosicao(posicao);
  }
